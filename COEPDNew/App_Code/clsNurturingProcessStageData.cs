...
{
    public class clsNurturingProcessStageData
    {
        public string Constr = ConfigurationManager.ConnectionStrings["cs"].ConnectionString.ToString();
        public virtual void AddUpdate(clsNurturingProcessStage obj)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("NurturingProcessStage_AddUpdate", objConn))
                {
                    objConn.Open();
        ...
        public virtual clsNurturingProcessStage Load(int Id)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("NurturingProcessStage_Get", objConn))
                {
                    objConn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    objCmd.Parameters.AddWithValue("NurturingProcessStageId", Id);
                    ...
                    if (objDset.Tables.Count > 0)
                    {
                        ...
                        {
                            clsNurturingProcessStage objInfo = new clsNurturingProcessStage();
                            objInfo.NurturingProcessStageId = Convert.ToInt32(objDset.Tables[0].Rows[0].ItemArray[0]);
                            objInfo.NurturingProcessStageName = Convert.ToString(objDset.Tables[0].Rows[0].ItemArray[1]);
                            return objInfo;
                        }
                    }
                    return null;
        ...
        public virtual List<clsNurturingProcessStage> LoadAll(clsNurturingProcessStage obj)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("NurturingProcessStage_List", objConn))
                {
                    objConn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    objCmd.Parameters.AddWithValue("@NurturingProcessStageName", obj.NurturingProcessStageName);
                    ...
                    if (objDset.Tables.Count > 0)
                    {
                        if (objDset.Tables[0].Rows.Count > 0)
                        {
                            List<clsNurturingProcessStage> objList = new List<clsNurturingProcessStage>();
                            for (int cnt = 0; cnt < objDset.Tables[0].Rows.Count; cnt++)
                            {
                                clsNurturingProcessStage objInfo = new clsNurturingProcessStage();
                                objInfo.SNo = cnt + 1;
                                objInfo.NurturingProcessStageId = Convert.ToInt32(objDset.Tables[0].Rows[cnt].ItemArray[0]);
        ...
        public void Remove(int id)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("NurturingProcessStage_Remove", objConn))
                {
                    objConn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    objCmd.Parameters.AddWithValue("NurturingProcessStageId", id);
        ...
        public int LoadNurturingProcessStageValidation(clsNurturingProcessStage obj)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("NurturingProcessStage_Validation", objConn))
                {
                    objConn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    objCmd.Parameters.AddWithValue("@NurturingProcessStageName", obj.NurturingProcessStageName);
