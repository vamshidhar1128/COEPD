...
{
    public class clsDocCategoryData
    {
        public string Constr = ConfigurationManager.ConnectionStrings["cs"].ConnectionString.ToString();

        public void AddUpdate(clsDocCategory obj)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("DocCategory_AddUpdate", objConn))
                {
                    objConn.Open();
        ...
        public clsDocCategory Load(int Id)
        {
            using (SqlConnection objconn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("DocCategory_Get", objconn))
                {
                    objconn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    ...
                    if (objDset.Tables.Count > 0)
                    {
                        ...
                        {
                            clsDocCategory objInfo = new clsDocCategory();
                            ...
                            objInfo.ParentCategoryId = Convert.ToInt32(objDset.Tables[0].Rows[0].ItemArray[3]);
        ...
        public List<clsDocCategory> LoadAll(clsDocCategory obj)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("DocCategory_List", objConn))
                {
                    objConn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    ...
                    if (objDset.Tables.Count > 0)
                    {
                        if (objDset.Tables[0].Rows.Count > 0)
                        {
                            List<clsDocCategory> objList = new List<clsDocCategory>();
                            for (int cnt = 0; cnt < objDset.Tables[0].Rows.Count; cnt++)
                            {
                                ...
                                objInfo.ParentCategoryId = Convert.ToInt32(objDset.Tables[0].Rows[cnt].ItemArray[3]);
        ...
        public void Remove(int Id)
        {
            using (SqlConnection objConn = new SqlConnection(Constr))
            {
                using (SqlCommand objCmd = new SqlCommand("DocCategory_Remove", objConn))
                {
                    objConn.Open();
                    objCmd.CommandType = CommandType.StoredProcedure;
                    objCmd.Parameters.AddWithValue("@CategoryId", Id);
